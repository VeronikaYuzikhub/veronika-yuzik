// Loading animation — flower bloom with sparkle ring (~3.5s), then fades out
...
export class FlowerLoader {
  ...
  private build(): void {
    ...
    // 14 gems fly to center
    for (let i = 0; i < 14; i++) {
      const a = (i / 14) * Math.PI * 2
      this.gems.push({
        sx: CX + Math.cos(a + Math.PI) * 88 + (Math.random() - 0.5) * 26,
        sy: CY + Math.sin(a + Math.PI) * 88 + (Math.random() - 0.5) * 26,
        tx: CX + Math.cos(a) * 19,
        ty: CY + Math.sin(a) * 19,
        delay: 0.50 + i * 0.022,
  ...
  private draw(): void {
    ...
    ag.addColorStop(0, `rgba(212,168,184,${p * 0.13})`)
    ag.addColorStop(0.55, `rgba(168,196,212,${p * 0.07})`)
    ag.addColorStop(1, 'transparent')
    ctx.fillStyle = ag; ctx.beginPath(); ctx.arc(CX, CY, p * 85, 0, Math.PI * 2); ctx.fill()

    // Petals
    this.petals.forEach(pt => {
      const t = Math.max(0, Math.min(1, (p - pt.delay) / 0.58)); if (!t) return
      const et = easeOut(t)
      ...
      ctx.strokeStyle = col + (0.22 * et) + ')'; ctx.lineWidth = 0.5
    ...
    this.sparks.forEach(sp => {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        ctx.lineTo(sp.x + c90 * s * 0.3, sp.y + s90 * s * 0.3)
    ...
    // Centre core
    const cT = Math.max(0, Math.min(1, (p - 0.72) / 0.28))
    if (cT > 0) {
      const et = easeOut(cT), cR = 12 * et
      const cg = ctx.createRadialGradient(CX, CY, 0, CX, CY, cR)
      cg.addColorStop(0, 'rgba(255,255,255,0.96)')
      cg.addColorStop(0.5, 'rgba(212,168,184,0.72)')
      ...
      ctx.strokeStyle = `rgba(255,255,255,${0.45 * et})`; ctx.lineWidth = 0.9; ctx.stroke()
    }
  }
